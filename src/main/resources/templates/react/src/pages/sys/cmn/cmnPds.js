...
import CmnPdsModal from './cmnPdsModal';
...
const CmnPds = () => {
  ...
  const [pdsSeq, setPdsSeq] = useState(0);
  ...
  const fetchPdss = async (page, size = perPage) => {
    setLoading(true);
    let params = {
      searchWrd: searchWrd,
      useYn: '',
  ...
  const handlePageChange = (page) => {
    fetchPdss(page).then((r) => setCurrentPage(page));
  };

  const handlePerRowsChange = (newPerPage, page) => {
    fetchPdss(page, newPerPage).then((r) => setPerPage(newPerPage));
  };
  ...
  return (
    <div>
      <ol className='breadcrumb float-xl-end me-3'>
        ...
        <li className='breadcrumb-item active'>자료실 관리</li>
      </ol>
      <h1 className='page-header ms-3 mb-2 pt-2'>자료실 관리</h1>
      <Panel>
        <PanelBody>
          ...
          <div className='row mt-3'>
            ...
            <div className=' col-md-1   px-2 justify-content-end d-flex'>
              <div className='text-end justify-content-start'>
      ...
      <CmnPdsModal show={isViewModal} toggleShow={handleToggleShow} reloadParentList={fetchPdss} seq={pdsSeq} />
    </div>
  );
};

export default CmnPds;
