...
const CmnPdsModal = (props) => {
  ...
  // 라이프사이클 끝
  ////////////////////////////////////////////////////////////////////////////////////////////////////////
  const fetchPds = async (seq) => {
    await agent.BrdInfo.fetchPds(seq).then(({ resultData, result, cmnFileList }) => {
      if (result === 'success') {
  ...
  const postPds = async () => {
    ...
    // formik.values의 각 항목을 FormData에 추가
    ...
    if (files2.length > 0) {
      for (let i = 0; i < files2.length; i++) {
        ...
        formData.append(`cmnFileDomainList[${i}].brdType`, 'PDS');
      }
    }

    agent.BrdInfo.postPds(formData)
      .then((data) => {
  ...
  const handleDelete = () => {
    if (window.confirm('삭제하시겠습니까?')) {
      agent.BrdInfo.deletePds(seq).then(({ result }) => {
        if (result === 'success') {
          alert('삭제되었습니다.');
          reloadParentList(1);
          toggleShow();
  ...
  return (
    <Modal size='xl' show={show} onHide={handleClose} backdrop='static' keyboard={false} onEnter={handleResetData}>
      ...
      <Modal.Footer>
        <button className='btn btn-danger me-5' onClick={handleDelete}>삭제</button>
        <div>
          <button className='btn btn-white w-70px me-2' onClick={handleClose}>닫기</button>
          <button type='submit' className='btn btn-success w-70px' onClick={handleSubmit}>저장</button>
        </div>
      </Modal.Footer>
    </Modal>
  );
